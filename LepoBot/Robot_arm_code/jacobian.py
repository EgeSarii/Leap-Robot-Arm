...
def main():
  '''Given a six degree of freedom robotic arm and a desired end position of the end effector,
     calculate the five joint angles (i.e. servo angles).
  '''
 
  # A 2D array that lists the different axes of rotation (rows) for each joint
  # Here I assume our robotic arm has five joints, but you can add more if you like.
  # k = kx, ky, kz  
  k = np.array([[0,0,1],[0,1,0],[0,1,0]])
  ...
  t = np.array([[0,0,0],[0,0,a1],[a2,0,0]]) 
  #t = np.array([[0,0,0],[0,0,a1],[a2,0,0],[-11,0,9], [a4,0,0]]) 
 
  # Position of end effector in joint 5 (i.e. the last joint) frame
  p_eff_5 = [a3+a4+a5,0,0]
     
  # Create an object of the RoboticArm class
  k_c = RoboticArm(k,t)
 
  # Starting joint angles in radians (joint 1, joint 2, joint 3, joint 4, joint 5)
  #q_0 = np.array([90,180,70,140,60])
  #q_0 = np.array([1.5707963268,3.14159265,1.308996939])
                 #, 2.4434609528, 1.0471975512])
  #q_0 = np.array([0,0,1.308996939, 2.4434609528,1.0471975512])
  q_0 = np.array([0,0,0])
     
  # desired end position for the end effector with respect to the base frame of the robotic arm
  #endeffector_goal_position = np.array([a2+a3+a4+a5,0,a1])
  endeffector_goal_position = np.array([0,0,a1+a2+a3+a4+a5])
  #endeffector_goal_position = np.array([19, 0, 15.5])
  #endeffector_goal_position = np.array([30,0,5])
  #endeffector_goal_position = np.array([30,3,25])
  ...
  print(f'end_effector = {k_c.position(q_0,index=-1,p_i=p_eff_5)}')
  ...
  final_q = k_c.pseudo_inverse(q_0, p_eff_N=p_eff_5, goal_position=endeffector_goal_position, max_steps=500)
     
  # Final Joint Angles in degrees   
  print('\n\nFinal Joint Angles in Degrees')
  print(f'servo1= Servo(0, 16, ({np.degrees(final_q[0])})) \nservo2 = Servo(1, 17, ({np.degrees(final_q[1])}))\n'
        f'servo3= Servo(2, 18, {np.degrees(final_q[2])})')
        #\nservo4 = Servo(3, 19, ({np.degrees(final_q[3])}))\n'
        #)


  ''' print(f'servo1.set_angle(90+ {np.degrees(final_q[0])}) \nservo2.set_angle(180 -(360-{np.degrees(final_q[1])}/2))\n'
        f'servo3.set_angle(90--{np.degrees(final_q[2])}) \nservo4.set_angle(60+ {np.degrees(final_q[3])})\n'
        f'servo5.set_angle(60 +{np.degrees(final_q[4])})')'''
    #,f'Joint 5: {np.degrees(final_q[4])}'
